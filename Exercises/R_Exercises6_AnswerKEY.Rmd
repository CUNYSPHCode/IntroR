...
## create a new variable instead of overwritting
babies$smokeNA <- babies$smoke
## change 9s to NA
babies[babies$smoke == 9, "smokeNA"] <- NA
## check the result with a freq. table
table(babies$smokeNA, useNA = "always")

t.test(bwt ~ smokeNA, data = babies)
```
...
```{r}
babies.fit <- lm(bwt ~ gestation + smokeNA + age, data = babies)
