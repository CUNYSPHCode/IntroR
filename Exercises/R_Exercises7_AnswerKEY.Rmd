...
pew <- read_csv("~/GitHub/IntroR/Data/pew.csv")
pew %>%
...
## Method 2
nums <- pew[, -which(names(pew) == "religion")]
cbind.data.frame(religion = pew$religion, totals = rowSums(nums))

## Method 3
nums <- pew[, -which(names(pew) == "religion")]
cbind.data.frame(religion = pew$religion, totals = apply(nums, 1L, sum))
```
...
```{r}
## Method 1
flights1$month2 <- factor(flights1$month, levels = 1:12, labels = month.name)
